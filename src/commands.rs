...
use crate::builtins::{self, meta, moderation};
pub async fn run(context: Context, command: ApplicationCommandInteraction)
{
    ...
    let content = match command_name
    {
        ...
        "ban" =>
        {
            ...
            moderation::ban::run(&context, &guild_id, &user, reason, days).await;
        ...
        "warn" =>
        {
            ...
            moderation::warn::warn(&guild_id, user, reason)
        }
        ...
        "get_warnings" =>
        {
            ...
            moderation::warn::get_warns(&guild_id, user)
        }
        ...
        "remove_warns" =>
        {
            ...
            moderation::warn::remove_warns(&guild_id, user)
        }

        "timeout" =>
        {
            let mut user = None;
            let mut time: moderation::timeout::TimeoutTime = moderation::timeout::TimeoutTime::default();
            for option in command.data.options.clone()
            {
                let opt = option.resolved.unwrap();
        ...
        "release" =>
        {
            ...
            moderation::timeout::release(&context, &guild_id, user).await
        }
